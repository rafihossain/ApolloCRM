...
//            ->select(DB::raw($yr.'.event_date as ev_date,old_events.event_name as event_name, old_events.id as event_id, athletes.image as image, '.$yr.'.overall_rank as overall_rank,'.$yr.'.category_rank as category_rank,'.$yr.'.age_category as age_category, '.$yr.'.bib as bib, '.$yr.'.distance as distance,'.$yr.'.speed as speed,'.$yr.'.time as time,'.$yr.'.points as points'))
    ...
    {
      $query["age"] = Age_group::groupBy('group_title')->get();
    ...
    $myarray = [];
    foreach($data["data"] as $ath){
    
    $player = $ath->athlete_name;
    
    $athname = $ath->athlete_name;
    
    $current_overall_rank_status = DB::table("overall_rankings")->where('athlete_name',$athname)->orderBy('event_date', 'DESC')->first();
    if(!empty($current_overall_rank_status)){
         $yr = date('Y', strtotime($current_overall_rank_status->event_date));
    $type = $current_overall_rank_status->age_type;
    $myvalue = $this->serachResultAth($yr,$type,$player);

    // $ath->overall_wins = $myvalue['overall_wins'];
    // $ath->top3_finishes = $myvalue['top3_finishes'];
    // $ath->top10_finishes = $myvalue['top10_finishes'];
    // $ath->category_wins = $myvalue['category_wins'];
    // $ath->total_races = $myvalue['total_races'];
    $ath->current_overall_rank_status = $myvalue['current_overall_rank_status'];
    $ath->current_category_rank_status = $myvalue['current_category_rank_status'];
    $ath->current_overall_rank = $myvalue['current_overall_rank'];
    $ath->current_category_rank = $myvalue['current_category_rank'];
    $ath->mrp = $myvalue['mrp'];
    $ath->arp = $myvalue['arp'];
    $myarray[]= $ath;
    }
    ...
    public function serachResultAth($myr,$type,$player){
    ...
    public function personal_best($type , $player , $year)
    {
      ...
      $var =array();
       ...
       foreach ($year2 as $key => $year) {
         ...
         \DB::enableQueryLog();
          $data = DB::table($year)->where('athlete', $player)->groupBy('distance')->orderBy('distance', 'ASC')->get();
          foreach ($data as $key => $value) {
            $mye[] =$value->event_date .' '.$year;
...
public function callFrobadge($player , $type , $year, $events = ''){
 ...
 $year_selected= str_replace("s","", $yearss) - 1;
   ...
   foreach ($last_achievment as $key => $value) {
     foreach ($value as $key => $value2) {
       ...
       if ( date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2020 ) {
        $BadgeConsistency2021Increment++;
          if ($BadgeConsistency2021Increment >= $total_races_previous_year_event * 67 ) {
             $BadgeConsistency2021 = 1;
           }
        }
...
//if (date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2020) { (pijush)
       if (date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
      ...
      if ( (strpos($value2->event_name , "Urataka" ) !==false) && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
          $Urataka = 1;
        }

      if ( (strpos($value2->event_name , "Betico" ) !==false) && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
          $Betico = 1;
        } 
        
      if ( (strpos($value2->event_name , "Half" ) !==false) && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
          $Half = 1;
        }

      if ( (strpos($value2->event_name , "Ronde" ) !==false) && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
          $Ronde = 1;
        } 
        
      if ( (strpos($value2->event_name , "Turibana" ) !==false) && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
          $Turibana = 1;
        }

      if ( (strpos($value2->event_name , "Brandweer" ) !==false) && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
          $Brandweer = 1;
        }
        ...
        if ( (strpos($value2->event_name , "Urataka" ) !==false ) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
          $Chasing5KQuest_aarray[] = 1;
        }

        if ( (strpos($value2->event_name , "Aruba Bank 1" ) !==false) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021 ) {
            $Chasing5KQuest_aarray[] = 1;
          } 
          
        if ( (strpos($value2->event_name , "Aruba Bank 2" ) !==false) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021) {
            $Chasing5KQuest_aarray[] = 1;
          }

        if ( (strpos($value2->event_name , "Aruba Bank 3" ) !==false) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021) {
            $Chasing5KQuest_aarray[] = 1;
          } 
          
        if ( (strpos($value2->event_name , "Aruba Bank 4" ) !==false) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021) {
            $Chasing5KQuest_aarray[] = 1;
          }

        if ( (strpos($value2->event_name , "Run Back 2 School" ) !==false) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021) {
            $Chasing5KQuest_aarray[] = 1;
          }

        if ( (strpos($value2->event_name , "Sport Caribe" ) !==false) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021) {
            $Chasing5KQuest_aarray[] = 1;
          }

        if ( (strpos($value2->event_name , "Infinite" ) !==false) && $value2->distance >= 5  && date("Y", strtotime($value2->event_date)) == ($year_selected+1) && ($year_selected+1) >=2021) {
            $Chasing5KQuest_aarray[] = 1;
          } 
   ...
   if ($year_selected >2017) {
     ...
     //echo $year_selected ;  die();
      ...
      if ( ( $total_races_previous_year_event * .8 ) <= $BadgeConsistency2020Increment  && ($year_selected+1) >=2020 ) {
        //echo $year_selected ;die();
        $BadgeSuperConsistent = 1;
        }
      ...
      if($year_selected == 2021){
        //echo $year_selected;die();
      if ( ( $total_races_previous_year_event * .8 ) <= $BadgeConsistency2021Increment  && ($year_selected+1) >=2020 ) {
        $BadgeSuperConsistent = 1;
        }
...
return $data;
    ...
    public function personal_badge_show($player , $type , $year, $events = '')
    {
      ...
      $aaa = array_values(array_filter($last_achievment));
      $myarray = [];
      $ttype0 = [];
      $ttype1 = [];
      $ttype2 = [];
      ...
      $max0 = '';
      if(count($ttype0) > 0){
        $max0 = max(array_column($ttype0, 'event_date'));
        $yy = date('Y', strtotime($max0));
        $yr = 'eventdata_'. $yy.'s';
        $ttype =0;
        $myarray[] = $this->callFrobadge($player,$ttype,$yr);

      }
      $max1 = '';
      if(count($ttype1) > 0){
        $max1 = max(array_column($ttype1, 'event_date'));
        $yy = date('Y', strtotime($max1));
        $yr = 'eventdata_'. $yy.'s';
        $ttype =1;
        $myarray[] = $this->callFrobadge($player,$ttype,$yr);
      }
      $max2 = '';
      if(count($ttype2) > 0){
        $max2 = max(array_column($ttype2, 'event_date'));
        $yy = date('Y', strtotime($max2));
        $yr = 'eventdata_'. $yy.'s';
        $ttype =2;
        $myarray[] = $this->callFrobadge($player,$ttype,$yr);
      }
...
$data['Chasing5KQuest'] = $myarray[0]['Chasing5KQuest'] + @$myarray[1]['Chasing5KQuest'] + @$myarray[2]['Chasing5KQuest'] ;
$data['ClassicQuest'] = $myarray[0]['ClassicQuest'] + @$myarray[1]['ClassicQuest'] + @$myarray[2]['ClassicQuest'];
$data['pledgeCampaignBadg'] = $myarray[0]['pledgeCampaignBadg'] + @$myarray[1]['pledgeCampaignBadg'] + @$myarray[2]['pledgeCampaignBadg'] ;
$data['ThreeYearBadge'] = $myarray[0]['ThreeYearBadge'] + @$myarray[1]['ThreeYearBadge'] + @$myarray[2]['ThreeYearBadge'];
$data['BadgeConsistency2019'] = $myarray[0]['BadgeConsistency2019'] + @$myarray[1]['BadgeConsistency2019'] + @$myarray[2]['BadgeConsistency2019'] ;
$data['BadgeSuperConsistent'] = $myarray[0]['BadgeSuperConsistent'] + @$myarray[1]['BadgeSuperConsistent'] + @$myarray[2] ['BadgeSuperConsistent'];

$data['BadgeConsistency2020'] = $myarray[0]['BadgeConsistency2020'] + @$myarray[1]['BadgeConsistency2020'] + @$myarray[2] ['BadgeConsistency2020'];
$data['badge_3k'] = $myarray[0]['badge_3k'] + @$myarray[1]['badge_3k'] + @$myarray[2] ['badge_3k'];
$data['badge_5k'] = $myarray[0]['badge_5k'] + @$myarray[1]['badge_5k'] + @$myarray[2] ['badge_5k'];
$data['badge_10k'] = $myarray[0]['badge_10k'] + @$myarray[1]['badge_10k'] + @$myarray[2] ['badge_10k'];
$data['badge_21k'] = $myarray[0]['badge_21k'] + @$myarray[1]['badge_21k'] + @$myarray[2] ['badge_21k'];
$data['badge_42k'] = $myarray[0]['badge_42k'] + @$myarray[1]['badge_42k'] + @$myarray[2] ['badge_42k'];
$data['ultra_merathon'] = $myarray[0]['ultra_merathon'] + @$myarray[1]['ultra_merathon'] + @$myarray[2] ['ultra_merathon'];
