...
<div class="card">
    <div class="card-body overflowhidden">
        ...
        <ul class="nav nav-tabs nav-bordered border-0">
            ...
            <li class="nav-item">
                <a href="{{ route('backend.customfield-partner') }}" class="nav-link">
                    Partner
                </a>
            </li>
            <li class="nav-item">
            ...
            <li class="nav-item">
                <a href="{{ route('backend.customfield-application') }}" class="nav-link active">
                    Application
                </a>
            </li>
        </ul>
            ...
            <table id="datatable" class="table table-bordered dt-responsive table-responsive nowrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @foreach($customField->workflow as $value)
                                @if(isset($value->service_workflow))
                                    <button type="button" class="btn btn-light btn-sm">{{ $value->service_workflow }}</button>
                                @endif
                            @endforeach
                        </td>
                        <td>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2">
                        <div class="col-md-12">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-primary" id="addNewWorkflow"> + Add New Workflow</button>
                        </div>
                        <span class="text-danger" id="fieldError"></span>
                    </div>
                    <div class="mb-2">
...
<script>
       $(document).ready(function() {
        ...
        $('.editCustomFieldModel').on("click", function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var workflow = '';
                    for (var i = 0; i < response.customfield.workflow.length; i++) {
                        var value = '';
                        for (var j = 0; j < response.workflows.length; j++) {
                            if(response.customfield.workflow[i].id == response.workflows[j].id ){
                                var selected = 'selected';
                            }else{
                                var selected = '';
                            }
                            value += '<option value="'+response.workflows[j].id+'" '+selected+'>'+response.workflows[j].service_workflow+'</option>';
                        }
                        workflow += '<div class="row mb-2 mt-2"><div class="col-md-10"><select class="form-control mb-2" name="workflow_id[]"><option value="">Select Field Type</option>'+value+'</select></div><div class="col-md-2"><a href="javascript:void(0)" class="btn btn-danger removetr"><i class="mdi mdi-delete-outline mdi-10px"></i></a></div></div>';
                    }
                    var html = '';
                    html += '<input type="hidden" name="custom_id" value="' + response.customfield.id + '"><div class="mb-2"><label class="form-label">Select Module <span class="text-danger">*</span></label><select class="form-control" name="module_id"><option value="">Select Module</option><option value="1">Client</option><option value="2">Partner</option><option value="3">Product</option><option value="4" selected>Application</option></select><span class="text-danger moduleError"></span></div><div class="mb-2"><label class="form-label">Section Name <span class="text-danger">*</span></label><select class="form-control" name="section_id"><option value="">Select Name</option>' + name + '</select><span class="text-danger sectionError"></span></div><div class="mb-2"><label class="form-label">Custom Field Name <span class="text-danger">*</span></label><input type="text" class="form-control" name="field_name" value="' + response.customfield.field_name + '"><span class="text-danger fieldnameError"></span></div><div class="mb-2"><label class="form-label">Select Field Type <span class="text-danger">*</span></label><select class="form-control" name="field_id"><option value="">Select Field Type</option>' + field + '</select><span class="text-danger fieldError"></span></div><div class="mb-2"><div class="col-md-12"><label class="form-label">Select Workflow <span class="text-danger">*</span></label><div class="workflowlist mb-2">'+workflow+'</div><button type="button" class="btn btn-sm btn-outline-primary addNewWorkflow"> + Add New Workflow</button></div><span class="text-danger" id="fieldError"></span></div><div class="mb-2"><div>' + mandatory + ' <label class="form-label" for="mthis"> Make this field mandatory</label></div><div> ' + listview + ' <label class="form-label" for="mthis2">Show this field on List View</label></div></div><button type="button" class="btn btn-primary px-5 w-100" id="update"> Update </button>';
                    ...
                    for(var i=0; i < response.workflows.length; i++){
                        workflowResult +='<option value="'+response.workflows[i].id+'">'+response.workflows[i].service_workflow+'</option>';
                    }
                    $(".addNewWorkflow").click(function() {
                        $(".workflowlist").append('<div class="row mb-2 mt-2"><div class="col-md-10"><select class="form-control" name="workflow_id[]"><option value="">Select Field Type</option>'+workflowResult+'</select></div><div class="col-md-2"><a href="javascript:void(0)" class="btn btn-danger removetr"><i class="mdi mdi-delete-outline mdi-10px"></i></a></div> </div>');
                    });
                }
            });
        ...
        var workflow = <?= json_encode($workflows) ?>;
        // console.log(workflow);

        var workflowResult = '';
        for(var i=0; i < workflow.length; i++){
            workflowResult +='<option value="'+workflow[i].id+'">'+workflow[i].service_workflow+'</option>';
        }
