                    ...
                    <div class="card-body p-2">
                        <div class="dropdown float-end">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                ...
                                <a href="{{ route('backend.deleteproduct-promotion', ['id' => $promotion['id'], 'partner_id'=> $promotion['partner_id'] ]) }}" id="delete" class="dropdown-item">Delete</a>
                            </div>
                        </div>
                        <div class=" mb-3 ">
        ...
        <div class="modal-content">
            <form id="promotionFrom" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <input type="hidden" name="partner_id" value="{{ $product['partner_id'] }}">
                    <div class="mb-2">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#savePromotion").click(function(e) {
            ...
            $.ajax({
                url: "{{ route('backend.addproduct-promotion') }}",
                type: "POST",
                data: fromData,
                cache:false,
                contentType: false,
        ...
        $('.editPromotionModel').on("click", function() {
            ...
            $.ajax({
                url: "{{ route('backend.editproduct-promotion') }}",
                type: "POST",
                data: {
                ...
                success: function(response) {
                    ...
                    for(var i = 0; i < response.products.length; i++){
                        ...
                        for(var j=0; j < response.relatedProduct.length; j++){
                            console.log(response.products[i].id+'='+response.relatedProduct[j].id);

                            if(response.products[i].id == response.relatedProduct[j].id){
                                selected = 'selected';
                            }else{
                                selected = '';
                            }
                    ...
                    // if(response.promotion.apply_status == 1){
                    //     console.log('11');
                    //     var checked_one = '<input type="radio" id="customRadio1" name="apply_status" class="form-check-input" value="1" checked>';
                    // }

                    if(response.promotion.apply_status == 1){
                        var checked_one = 'checked';
                    }else{
                        var checked_one = '';
                    }

                    if(response.promotion.apply_status == 2){
                        var checked_two = 'checked';
                    }else{
                        var checked_two = '';
                    }
                    ...
                    var selected_value = response.promotion.apply_status;
                    if (selected_value == '2') {
                        $('.singleproduct').show();
                    } else {
                        $('.singleproduct').hide();
        ...
        $(document).delegate('#update', 'click', function(e) {
            ...
            $.ajax({
                url: "{{ route('backend.updateproduct-promotion') }}",
                type: "POST",
                data: fromData,
                cache:false,
                contentType: false,
        ...
        $('.viewPromotionModel').on("click", function(e) {
            ...
            $.ajax({
                url: "{{ route('backend.viewproduct-promotion') }}",
                type: "POST",
                data: {
                    promotion_id: promotionId,
                    partner_id: partnerId,
